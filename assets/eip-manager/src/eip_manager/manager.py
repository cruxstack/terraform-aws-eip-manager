...
- When a instances is stopped or terminated, the manager will remove all the
  EIPs that are associated with the instance.
- When a instance is started, the manager will add an Elastic IP address to
  the instance.
"""
import os
import boto3
...
from .config import LOG_LEVEL, POOL_TAG_KEY, POOL_TAG_VALUES, POOL_FILTER_ENABLED
    ...
    def add_addresses(self):
        ...
        for instance_id, network_interface_id, allocation_id in [
            (
                ...
                instances[i].primary_network_interface_id,
                allocation_ids[i],
            )
...
def get_all_pool_names() -> List[str]:
    ...
    paginator = resourcetagging.get_paginator("get_tag_values")
    pages = paginator.paginate(Key=POOL_TAG_KEY)
    for page in pages:
        tags = page["TagValues"]
        for tag in tags:
            if POOL_FILTER_ENABLED and "tag" not in POOL_TAG_VALUES:
                continue
